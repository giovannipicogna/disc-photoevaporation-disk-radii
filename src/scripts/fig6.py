...
ax_kde_age = fig.add_subplot(gs[1, 2])      # KDE age plot (bottom, under panel c)

# Main comparison panels with shared x-axis
ax_main = [
    fig.add_subplot(gs[0, 1], sharex=ax_kde_age),  # Panel a: Compact
    fig.add_subplot(gs[0, 2], sharex=ax_kde_age)   # Panel c: Extended
]
...
# Load additional population synthesis data for comparison
path = paths.data / 'pop_spreading/'
profile_name = "Internal Photoevaporation (Full)"
data_spreading = load_data(path, profile_name=profile_name, mask=mask,
                           mask_val=mask_val)

path = paths.data / 'pop_spreading_extern/'
profile_name = "Internal + External Photoevaporation (Full)"
data_spreading_extern = load_data(path, profile_name=profile_name, mask=mask,
                                   mask_val=mask_val)
...
print(f"Spreading data points: {len(data_spreading)}")
print(f"Spreading extern data points: {len(data_spreading_extern)}")
...
# Plot first panel: compact discs with standard PE rates (2D histogram)
h1 = ax_main[0].hist2d(data_compact['age'], data_compact['mdot_acc'],
               bins=[np.linspace(0, 20, 41), np.logspace(-13, -6, 41)],
               norm=LogNorm(), cmap='turbo')
plt.colorbar(h1[3], cax=cbar_ax_models)
...
ax_main[0].set_title(data_compact['profile'].iloc[0], fontsize=10)

# Plot second panel: extended discs (2D histogram)
ax_main[1].hist2d(data_extended['age'], data_extended['mdot_acc'],
                  bins=[np.linspace(0, 20, 41), np.logspace(-13, -6, 41)],
                  norm=LogNorm(), cmap='turbo')
...
# Remove y-axis label and tick labels for Panel b
...
ax_main[1].set_title(data_extended['profile'].iloc[0], fontsize=10)
...
# Create KDEs for additional datasets
log_mdot_spreading = np.log10(data_spreading['mdot_acc'][(data_spreading['mdot_acc'] >= 1e-13) & (data_spreading['mdot_acc'] <= 1e-6)])
log_mdot_spreading_extern = np.log10(data_spreading_extern['mdot_acc'][(data_spreading_extern['mdot_acc'] >= 1e-13) & (data_spreading_extern['mdot_acc'] <= 1e-6)])

if len(log_mdot_spreading) > 1:
    kde_spreading = gaussian_kde(log_mdot_spreading)
    kde_spreading_vals = kde_spreading(log_mdot_eval)
else:
    print(f"WARNING: Not enough Spreading data for KDE (found {len(log_mdot_spreading)} points), using zeros")
    kde_spreading_vals = np.zeros_like(log_mdot_eval)

if len(log_mdot_spreading_extern) > 1:
    kde_spreading_extern = gaussian_kde(log_mdot_spreading_extern)
    kde_spreading_extern_vals = kde_spreading_extern(log_mdot_eval)
else:
    print(f"WARNING: Not enough Spreading extern data for KDE (found {len(log_mdot_spreading_extern)} points), using zeros")
    kde_spreading_extern_vals = np.zeros_like(log_mdot_eval)

# Compute KDE for observed accretion rates
obs_mdot_mask = (acc_data['Macc'] > 0) & (acc_data['Macc'] >= 1e-13) & (acc_data['Macc'] <= 1e-6)
log_mdot_obs = np.log10(acc_data['Macc'][obs_mdot_mask])
if len(log_mdot_obs) > 1:
    kde_obs = gaussian_kde(log_mdot_obs)
    kde_obs_vals = kde_obs(log_mdot_eval)
else:
    print(f"WARNING: Not enough observed data for KDE (found {len(log_mdot_obs)} points), using zeros")
    kde_obs_vals = np.zeros_like(log_mdot_eval)

# Plot age distribution KDE differences below the right plot
ax_kde_age.plot(age_bin_centers, compact_age_dist, 'b-', linewidth=2, 
                label=data_compact['profile'].iloc[0], alpha=0.8)
ax_kde_age.plot(age_bin_centers, extended_age_dist, 'g-', linewidth=2, 
                label=data_extended['profile'].iloc[0], alpha=0.8)
ax_kde_age.fill_between(age_bin_centers, 
                        np.minimum(compact_age_dist, extended_age_dist),
                        np.maximum(compact_age_dist, extended_age_dist), 
...
ax_kde_mdot.plot(kde_compact_vals, mdot_eval, 'b-', linewidth=2, alpha=0.8)
ax_kde_mdot.plot(kde_extended_vals, mdot_eval, 'g-', linewidth=2, alpha=0.8)
ax_kde_mdot.plot(kde_spreading_vals, mdot_eval, 'b--', linewidth=1.5, alpha=0.7)
ax_kde_mdot.plot(kde_spreading_extern_vals, mdot_eval, 'g--', linewidth=1.5, alpha=0.7)
ax_kde_mdot.plot(kde_obs_vals, mdot_eval, 'k-', linewidth=2, alpha=0.9)
ax_kde_mdot.fill_betweenx(mdot_eval, 
                          np.minimum(kde_compact_vals, kde_extended_vals),
                          np.maximum(kde_compact_vals, kde_extended_vals), 
...
# Create dual legend system for mdot KDE
# Legend 1: Model colors
color_handles = [
    Line2D([0], [0], color='b', linewidth=2.0, alpha=0.8, label='internal'),
    Line2D([0], [0], color='g', linewidth=2.0, alpha=0.8, label='internal+ext'),
    Line2D([0], [0], color='k', linewidth=2.0, alpha=0.9, label='observations')
]
legend1 = ax_kde_mdot.legend(handles=color_handles, loc='upper right',
                             fontsize=5.5, framealpha=0.9, title='Model')

# Legend 2: Line styles for model variants
style_handles = [
    Line2D([0], [0], color='gray', linestyle='-', linewidth=2.0, label='Reduced'),
    Line2D([0], [0], color='gray', linestyle='--', linewidth=1.5, label='Full')
]
legend2 = ax_kde_mdot.legend(handles=style_handles, loc='lower right',
                             fontsize=5.5, framealpha=0.9, title='Internal PE')

# Add the first legend back (matplotlib removes it when adding the second)
ax_kde_mdot.add_artist(legend1)
