...
def load_mass_loss_data():
    ...
    directories = ['1Msun_Rcrit10au', '1Msun_Rcrit50au', '1Msun_Rcrit100au', '1Msun_Rcrit200au']
    labels = [r'$r_1 = 10$ au', r'$r_1 = 50$ au', 
              r'$r_1 = 100$ au', r'$r_1 = 200$ au']
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']  # Professional color scheme
    markers = ['o', 's', '^', 'v']  # Different markers for each simulation
    ...
    for i, directory in enumerate(directories):
        try:
            # Construct file path
            file_path = f'/e/ocean1/users/picogna/Photoevaporation-OuterRadius-Project/{directory}/mass_loss_fit_data.txt'
            ...
            print(f"Loaded {len(radius_au)} data points from {directory}")
            
        except FileNotFoundError:
            print(f"Warning: File not found for {directory}")
...
def create_publication_figure(data_dict, fitting_results=None, 
                            save_path='mass_loss_rate_comparison_publication.pdf',
    ...
    for directory, data in data_dict.items():
        ...
        # Plot with professional styling using data points
        ax.loglog(radius_filtered, mdot_filtered, 
                 color=data['color'], 
                 marker=data['marker'],
    ...
    if fitting_results is not None:
        ...
        if inner_results[1] is not None:
            ...
            # Plot extended fit with dashed gray line
            ax.loglog(
                r_inner_extended,
                mdot_inner_extended,
                color='gray',
                ...
                linewidth=1.5,
                alpha=0.7,
                label=f'Inner fit: $r^{{{alpha_inner:.2f}}}$ '
                      f'($r <$ {inner_limit:.0f} AU)',
                zorder=3
            )
        ...
        if outer_results[1] is not None:
            ...
            ax.loglog(
                ...
                linewidth=1.5,
                alpha=0.7,
                label=f'Outer fit: $r^{{{alpha_outer:.2f}}}$ '
                      f'($r >$ {outer_limit:.0f} AU)',
                zorder=3
            )
    ...
    print(f"Figure saved as {save_path.replace('.pdf', '.png')}")
