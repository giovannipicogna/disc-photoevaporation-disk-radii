"""
Publication-ready script to generate Figure 7 (Compact Discs): Critical radius distribution comparison.

This script creates a comprehensive figure comparing photoevaporation prescriptions
through the critical radius (r1) distribution as a function of time, focusing on
COMPACT DISCS ONLY (r1 < 30 AU).
...
# ============================================================================
# COMPACT DISC FILTERING (r1 < 20 AU)
# ============================================================================
print(f"\n" + "="*60)
print("APPLYING COMPACT DISC FILTER: r1 < 30 AU")
print("="*60)

# Apply compact disc filter to both datasets
compact_disc_limit = 30.0  # AU

print(f"\nBefore compact disc filter:")
print(f"  Internal photoevaporation: {len(data_internal)} data points")
print(f"  Internal + External photoevaporation: {len(data_external)} data points")

data_internal = data_internal[data_internal['r1'] < compact_disc_limit]
data_external = data_external[data_external['r1'] < compact_disc_limit]

print(f"\nAfter compact disc filter (r1 < {compact_disc_limit} AU):")
print(f"  Internal photoevaporation: {len(data_internal)} data points")
print(f"  Internal + External photoevaporation: {len(data_external)} data points")

# Print diagnostic information about loaded data
print(f"\nCompact disc data summary:")
if len(data_internal) > 0:
    ...
    print("WARNING: Internal dataset is empty after filtering!")
if len(data_external) > 0:
    ...
    print("WARNING: External dataset is empty after filtering!")

# ============================================================================
# PLOTTING EXECUTION
# ============================================================================
# Determine appropriate r1 limits for plotting (adjusted for compact discs)
# Focus on compact disc range
r1_min = 1e0  # AU - minimum for log scale
r1_max = compact_disc_limit   # AU - maximum is the compact disc limit
print(f"\nr1 range for plots: {r1_min:.3e} - {r1_max:.1f} AU (compact discs only)")

# Filter data to focus on compact disc radii
...
print(f"After filtering to {r1_min:.3e} - {r1_max:.1f} AU:")
print(f"  Internal: {len(data_internal_plot)} data points ({100*len(data_internal_plot)/max(1,len(data_internal)):.1f}%)")
print(f"  External: {len(data_external_plot)} data points ({100*len(data_external_plot)/max(1,len(data_external)):.1f}%)")

# Plot first panel: internal photoevaporation only (2D histogram)
h1 = ax_main[0].hist2d(data_internal_plot['age'], data_internal_plot['r1'],
               bins=[np.linspace(0, 20, 81), np.logspace(np.log10(r1_min), np.log10(r1_max), 81)],
...
ax_main[0].set_title('Internal photoevaporation', fontsize=10)

# Plot second panel: internal + external photoevaporation (2D histogram)
ax_main[1].hist2d(data_external_plot['age'], data_external_plot['r1'],
                  bins=[np.linspace(0, 20, 81), np.logspace(np.log10(r1_min), np.log10(r1_max), 81)],
                  norm=LogNorm(), cmap='turbo')
...
ax_main[1].set_title('Internal + External photoevaporation', fontsize=10)

# ============================================================================
# KDE AT SPECIFIC AGES ANALYSIS
# ============================================================================
print("\nComputing KDE at specific ages (compact discs only)...")

# Define ages of interest and time windows
ages_of_interest = [1.0, 3.0, 6.0]  # Myr
age_window = 0.25  # ±0.25 Myr window around each age

# Extract initial r1 distribution from parameters.csv (same approach as fig1.py)
# but filter for compact discs only
...
r1_initial = r1_initial[(r1_initial >= r1_min) & (r1_initial < compact_disc_limit)]
print(f"Initial r1 distribution: {len(r1_initial)} compact discs (r1 < {compact_disc_limit} AU) from initial conditions")
...
linestyles = {1.0: '-', 3.0: '--', 6.0: '-.'}
linewidths = {1.0: 2.0, 3.0: 1.8, 6.0: 1.6}

# Plot initial conditions (age-independent, shown once)
ax_kde_r1.plot(kde_initial_vals, r1_eval, 'k:', linewidth=1.5, alpha=0.7)
...
for age in ages_of_interest:
    ax_kde_r1.plot(kde_results['internal'][age], r1_eval, 
                   ...
                   linewidth=linewidths[age], alpha=0.8)
...
ax_kde_r1.fill_betweenx(r1_eval, 
                        kde_results['internal'][3.0],
                        kde_results['external'][3.0], 
...
# Legend 1: Line colors (models)
color_handles = [
    Line2D([0], [0], color='k', linestyle=':', linewidth=1.5, alpha=0.7, label='initial'),
    Line2D([0], [0], color='#0072B2', linewidth=2.0, alpha=0.8, label='internal'),
    Line2D([0], [0], color='#009E73', linewidth=2.0, alpha=0.8, label='internal+ext')
]
legend1 = ax_kde_r1.legend(handles=color_handles, loc='upper right', 
                           fontsize=5.5, framealpha=0.9, title='Model')

# Legend 2: Line styles (ages)
style_handles = [
    Line2D([0], [0], color='gray', linestyle='-', linewidth=2.0, label='1 Myr'),
    Line2D([0], [0], color='gray', linestyle='--', linewidth=1.8, label='3 Myr'),
    Line2D([0], [0], color='gray', linestyle='-.', linewidth=1.6, label='6 Myr')
]
legend2 = ax_kde_r1.legend(handles=style_handles, loc='center right', 
                           fontsize=5.5, framealpha=0.9, title='Age')

# Add the first legend back (matplotlib removes the first when adding the second)
ax_kde_r1.add_artist(legend1)
...
# SAVE FIGURE
# ============================================================================
output_path = paths.figures / 'Fig7_compact.png'
fig.savefig(output_path, format='png', bbox_inches='tight')
print(f"\nFigure saved as: {output_path}")

print("\nFig7_compact generation complete!")
print(f"Analysis focused on compact discs: r1 < {compact_disc_limit} AU")
